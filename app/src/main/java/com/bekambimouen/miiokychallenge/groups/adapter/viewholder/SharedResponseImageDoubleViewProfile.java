...
import com.bekambimouen.miiokychallenge.groups.full_image_share.GroupResponseImageDoubleFullImageShare;
...
public class SharedResponseImageDoubleViewProfile extends RecyclerView.ViewHolder {
    private static final String TAG = "SharedResponseImageDoubleViewProfile";
    ...
    public SharedResponseImageDoubleViewProfile(AppCompatActivity context, boolean isFrom_approval_post,
    ...
    public void init(BattleModel model) {
        ...
        if (time >= 2*3600*24000254025L)
            timeDetla.setText(TimeUtils.getTime(context, date_shared));
        else
            timeDetla.setText(Html.fromHtml(context.getString(R.string.there_is)+" "+ TimeUtils.getTime(context, date_shared)));
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot ds : dataSnapshot.getChildren()){
                    ...
                    relLayout_go_user_profile_un.setOnClickListener(v -> {
                        ...
                        context.getWindow().setEnterTransition(new Slide(Gravity.START));
                        Intent intent;
                        if (user_reponse.getUser_id().equals(requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid())) {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot ds: snapshot.getChildren()) {
                    ...
                    relLayout_go_user_profile_deux.setOnClickListener(v -> {
                        ...
                        context.getWindow().setEnterTransition(new Slide(Gravity.START));
                        Intent intent;
                        if (user_invite.getUser_id().equals(requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid())) {
                            intent = new Intent(context, Profile.class);
        ...
        GroupBottomSheetAddCommentShare bottomSheetFragmentShare = new GroupBottomSheetAddCommentShare(context,
                ...
                null, "response_img_double_single_bottom", null, null, null,
                null, null, null, null,
        ...
        bottomSheetShare_un = new BottomSheetSharePublication(context, model, model.getUser_id(),
                model.getReponse_url(), model.getReponse_photoID(), "", "", true);
        ...
        bottomSheetShare_deux = new BottomSheetSharePublication(context, model, model.getUser_id(),
                model.getInvite_url(),model.getInvite_photoID(), "", "", true);
    ...
    private final class GestureListenerLike_un extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onSingleTapConfirmed(@NonNull MotionEvent e) {
            ...
            context.getWindow().setExitTransition(new Slide(Gravity.END));
            context.getWindow().setEnterTransition(new Slide(Gravity.START));
            Intent intent = new Intent(context, GroupResponseImageDoubleFullImageShare.class);
            Gson gson = new Gson();
            String myGson = gson.toJson(photo);
            intent.putExtra("battleModel_response_image_double", myGson);
            intent.putExtra("response_photoID", photo.getReponse_photoID());
            intent.putExtra("position", 0);
            intent.putExtra("response_img_double_single_bottom", "response_img_double_single_bottom");
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot ds : dataSnapshot.getChildren()){
                    ...
                    if(likedByCurrentUser_un
                        ...
                        assert keyID != null;
                        mReference.child(context.getString(R.string.dbname_group_media_share))
                                .child(photo.getPhoto_id())
                                .child(context.getString(R.string.field_likes_share_un))
                                .child(keyID)
                                .removeValue();
        ...
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot ds: snapshot.getChildren()) {
                    ...
                    crusher_list_un.add(requireNonNull(requireNonNull(ds.child(context.getString(R.string.field_user_id)).getValue())).toString());
                    crush_count_un++;
                }
