...
public class PrepareNextVideoChallenge {
    private static final String TAG = "PrepareNextVideoChallenge";
    private static final int CONNECTION_TIMEOUT = 5000; // 5 seconds

    public interface PreloadListener {
        void onPreloadProgress(ModelInvite videoItem, int progress);
        void onPreloadComplete(ModelInvite videoItem);
        void onPreloadError(ModelInvite videoItem, Exception e);
    }

    public static void preDownloadVideo(Context context, ModelInvite videoItem, PreloadListener listener) {
        HttpProxyCacheServer proxy = App.getProxy(context);
        String proxyUrl = proxy.getProxyUrl(videoItem.getInvite_url());
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (proxyUrl.startsWith("file://")) {
                    Log.d(TAG, "preDownloadVideo: Handling file:// URL for " + videoItem.getInvite_url());
                    ...
                    Log.d(TAG, "preDownloadVideo: Handling http:// URL for " + videoItem.getInvite_url());
                    ...
                    Log.d(TAG, "preDownloadVideo: Preload complete for " + videoItem.getInvite_url());
                } else {
                ...
                Log.d(TAG, "preDownloadVideo: Error preloading " + videoItem.getInvite_url(), e);
                if (listener != null) {
