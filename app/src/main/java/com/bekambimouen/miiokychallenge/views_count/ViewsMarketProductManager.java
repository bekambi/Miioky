...
public class ViewsMarketProductManager {

    private final AppCompatActivity context;
    private final DatabaseReference databaseReference;

    public ViewsMarketProductManager(AppCompatActivity context) {
        this.context = context;

        databaseReference = FirebaseDatabase.getInstance().getReference();
    }

    public void incrementViewCount(String user_or_store_id, String publicationId) {
        DatabaseReference publicationRef1 = databaseReference
                .child(context.getString(R.string.dbname_market))
                .child(user_or_store_id)
                .child(publicationId);
